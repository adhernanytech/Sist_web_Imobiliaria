// Imóveis fixos (pré-cadastrados)
const imoveisFixos = {
  talatona_apt: {
    titulo: "Apartamento T3 em Talatona",
    tipologia: "T3",
    localizacao: "Talatona",
    descricao: "Apartamento moderno com suíte e varanda, localizado em condomínio fechado.",
...
// Recupera imóveis do localStorage
const imoveisStorage = JSON.parse(localStorage.getItem("imoveis")) || {};

// Junta os dois (fixos + cadastrados pelo admin)
const imoveis = { ...imoveisFixos, ...imoveisStorage };
...
for (const [id, imovel] of Object.entries(imoveis)) {
  ...
  col.innerHTML = `
    <div class="card h-100 shadow-sm">
      <img src="${imovel.imagem}" class="card-img-top" alt="${imovel.titulo}" onclick="abrirModal('${id}')">
      <div class="card-body">
        <h5 class="card-title" style="color: rgba(231, 94, 2, 0.904)">${imovel.titulo}</h5>
        <p class="card-text"><strong>Tipologia:</strong> ${imovel.tipologia}</p>
        <p class="card-text"><strong>Localização:</strong> ${imovel.localizacao}</p>
